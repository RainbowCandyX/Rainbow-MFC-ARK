...
BOOL CProcessInfoDlg::OnInitDialog()
{
	...
	CString szImage;
	szImage.Format(L"%s ( %s | PID:%d | 0x%llX )", L"进程信息", m_szImageName, m_nPid, m_pEprocess);
	...
	m_tab.AddPage(L"线程", &m_threadDlg, IDD_THREAD_DIALOG);


	m_vadDlg.m_dwPid = (ULONG)m_nPid;
	m_tab.AddPage(L"VAD信息", &m_vadDlg, IDD_VAD_DIALOG);

	m_moduleDlg.m_dwPid = m_nPid;
	m_moduleDlg.m_pEprocess = (ULONG_PTR)m_pEprocess;
	m_tab.AddPage(L"模块", &m_moduleDlg, IDD_MODULE_DIALOG);

	m_memoryDlg.m_dwPid = m_nPid;
	m_memoryDlg.m_pEprocess = (ULONG_PTR)m_pEprocess;
	m_tab.AddPage(L"内存", &m_memoryDlg, IDD_MEMORY_DIALOG);


	m_handleDlg.m_dwPid = m_nPid;
	m_tab.AddPage(L"句柄", &m_handleDlg, IDD_HANDLE_DIALOG);


	m_wndDlg.m_dwPid = (ULONG)m_nPid;
	m_tab.AddPage(L"窗口", &m_wndDlg, IDD_HWND_DIALOG);

	m_tokenDlg.m_dwPid = (ULONG)m_nPid;
	m_tab.AddPage(L"令牌", &m_tokenDlg, IDD_TOKEN_DIALOG);
