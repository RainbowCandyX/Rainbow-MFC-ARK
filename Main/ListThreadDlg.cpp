...
BOOL CListThreadDlg::OnInitDialog()
{
	...
	m_threadList.InsertColumn(eThreadId, L"线程ID", LVCFMT_LEFT, 50);
	m_threadList.InsertColumn(eThreadObject, L"线程对象", LVCFMT_LEFT, 85);
	m_threadList.InsertColumn(eThreadTeb, L"TEB", LVCFMT_LEFT, 85);
	m_threadList.InsertColumn(eThreadPriority, L"优先级", LVCFMT_LEFT, 85);
	m_threadList.InsertColumn(eThreadStartAddress, L"起始地址", LVCFMT_LEFT, 85);
	m_threadList.InsertColumn(eThreadSwitchTimes, L"切换次数", LVCFMT_LEFT, 85);
	m_threadList.InsertColumn(eThreadStatus, L"状态", LVCFMT_LEFT, 85);
	m_threadList.InsertColumn(eThreadFlag, L"暂停计数", LVCFMT_CENTER, 100);
	m_threadList.InsertColumn(eThreadStartModule, L"起始模块", LVCFMT_LEFT, 520);


	m_threadInfo.Format(L"线程数量:%d Hide:%d", m_nThreadsCnt, m_hideThreadCount);
