...
BOOL CListVadDlg::OnInitDialog()
{
	...
	m_listVad.InsertColumn(eStart, L"起始地址", LVCFMT_LEFT, 125);
	m_listVad.InsertColumn(eEnd, L"结束地址", LVCFMT_LEFT, 125);
	m_listVad.InsertColumn(eType, L"类型", LVCFMT_LEFT, 70);
	m_listVad.InsertColumn(eProtect, L"保护", LVCFMT_LEFT, 150);
	m_listVad.InsertColumn(eSection, L"节点", LVCFMT_LEFT, 500);
...
{
	if (MessageBox(L"对于系统DLL不要乱改(如ntdll,kernel32),除非你真的理解-Windows内存管理机制", L"警告",MB_OKCANCEL) == IDOK)
	{
