...
BOOL CListProcessDlg::OnInitDialog()
{
	...
	m_processlist.InsertColumn(eProcessImageName, L"进程名", LVCFMT_LEFT, 130);
	m_processlist.InsertColumn(eProcessPid, L"PID", LVCFMT_LEFT, 65);
	m_processlist.InsertColumn(eProcessParentPid, L"父进程PID", LVCFMT_LEFT, 65);
	m_processlist.InsertColumn(eProcessPath, L"进程路径", LVCFMT_LEFT, 290);
	m_processlist.InsertColumn(eProcessEprocess, L"EPROCESS", LVCFMT_LEFT, 80);
	m_processlist.InsertColumn(eProcessR3Access, L"应用层访问", LVCFMT_CENTER, 75);
	m_processlist.InsertColumn(eProcessFileCompany, L"公司", LVCFMT_LEFT, 140);
	m_processlist.InsertColumn(eProcessFileDescription, L"描述", LVCFMT_LEFT, 190);
	m_processlist.InsertColumn(eProcessCreateTime, L"创建时间", LVCFMT_LEFT, 140);
	m_processlist.InsertColumn(eProcessCommandLine, L"命令行", LVCFMT_LEFT, 140);
...
void CListProcessDlg::ShowProcessAsList()
{
	...
	m_szProcessCnt.Format(L"进程:%d, 隐藏进程:%d, 应用层无法访问进程:%d", m_nProcessCnt, m_nHideCnt, m_nDenyAccessCnt);
...
BOOL CListProcessDlg::CanOpenProcess(HANDLE dwPid)
{
	...
	HANDLE hProcess = OpenProcess(PROCESS_TERMINATE | PROCESS_VM_OPERATION, FALSE, (DWORD)dwPid);
