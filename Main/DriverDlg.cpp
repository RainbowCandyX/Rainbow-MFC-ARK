...
BOOL CListDriverDlg::OnInitDialog()
{
	...
	m_driverList.InsertColumn(eDriverName, L"驱动名", LVCFMT_LEFT, 120);
	m_driverList.InsertColumn(eDriverBase, L"地址", LVCFMT_LEFT, 100);
	m_driverList.InsertColumn(eDriverSize, L"大小", LVCFMT_LEFT, 100);
	m_driverList.InsertColumn(eDriverObject, L"驱动对象", LVCFMT_LEFT, 100);
	m_driverList.InsertColumn(eDriverPath, L"路径", LVCFMT_LEFT, 270);
	m_driverList.InsertColumn(eDriverServiceName, L"服务名", LVCFMT_LEFT, 110);
	m_driverList.InsertColumn(eDriverStartupType, L"启动类型", LVCFMT_CENTER, 80);
	m_driverList.InsertColumn(eDriverLoadOrder, L"加载顺序", LVCFMT_CENTER, 70);
	m_driverList.InsertColumn(eDriverFileCorporation, L"公司", LVCFMT_LEFT, 140);
	m_driverList.InsertColumn(eDriverFileDescription, L"描述", LVCFMT_LEFT, 200);
	m_driverInfo.Format(L"驱动数量:%d, 隐藏驱动:%d, 可疑对象:%d", 0, 0, 0);
...
void CListDriverDlg::InsertDriverIntem()
{
	...
	m_driverInfo.Format(L"驱动数量:%d, 隐藏驱动:%d, 可疑对象:%d", m_driverList.GetItemCount(), nHideDriverCnt, 0);
